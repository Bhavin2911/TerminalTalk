...
def update_display_orientation():
    """
    Automatically set LED matrix rotation based on current orientation.
    """
    ...
    # Determine closest orientation
    if roll < 45 or roll > 315:
...
def chat_mode():
    ...
    while True:
        ...
        if user_input.lower() == "timegame":
            update_display_orientation()
            show_symbol(GAME_START_SYMBOL)
            time_guess_game()
            print(f"{current_time()} Time game finished.")
            continue
...
def wait_for_joystick_press(timeout=40):
    """
    Wait for a joystick press with timeout.
    Returns True if pressed, False if timeout occurs.
    """
    if not SENSE_AVAILABLE:
        return False
...
def time_guess_game():
    """
    Time-based haptic game using Sense HAT joystick.
    """
    if not SENSE_AVAILABLE:
        print("Sense HAT not available.")
        return

    # Step 1: Wait for start press
    update_display_orientation()
    sense.show_message("Press!", scroll_speed=0.07)
    if not wait_for_joystick_press():
        show_temperature_idle()
        return

    # Step 2: Intro + random time
    target_seconds = random.randint(1, 20)
    update_display_orientation()
    sense.show_message(
        f"Guess {target_seconds}s",
        scroll_speed=0.07,
        text_colour=(0, 255, 0)
    )
    ...
    # Step 3: Wait for stop press (max 40s)
    if not wait_for_joystick_press(timeout=40):
        update_display_orientation()
    ...
    diff_ms = int(abs(elapsed - target_seconds) * 1000)
